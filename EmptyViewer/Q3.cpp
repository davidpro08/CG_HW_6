...
void render_phong() {
    ...
    std::vector<glm::vec3> cam_positions(vertices.size());
    std::vector<glm::vec3> normals(vertices.size());
    for (size_t i = 0; i < vertices.size(); ++i) {
        glm::vec4 cam_pos = MV * glm::vec4(vertices[i].position, 1.0f);
        cam_positions[i] = glm::vec3(cam_pos);
        normals[i] = glm::normalize(glm::mat3(MV) * vertices[i].normal);
    ...
    for (auto tri : triangles) {
        ...
        glm::vec3 c0 = cam_positions[tri.v0];
        glm::vec3 c1 = cam_positions[tri.v1];
        glm::vec3 c2 = cam_positions[tri.v2];
        glm::vec3 n0 = normals[tri.v0];
        glm::vec3 n1 = normals[tri.v1];
        glm::vec3 n2 = normals[tri.v2];
        ...
        for (int y = (int)ymin; y <= (int)ymax; ++y) {
            for (int x = (int)xmin; x <= (int)xmax; ++x) {
                glm::vec2 pixelCoord(x + 0.5f, y + 0.5f);
                glm::vec2 a(p0.x, p0.y);
                glm::vec2 b(p1.x, p1.y);
                glm::vec2 c(p2.x, p2.y);
                float denom = (b.y - c.y) * (a.x - c.x) + (c.x - b.x) * (a.y - c.y);
                float alpha = ((b.y - c.y) * (pixelCoord.x - c.x) + (c.x - b.x) * (pixelCoord.y - c.y)) / denom;
                float beta = ((c.y - a.y) * (pixelCoord.x - c.x) + (a.x - c.x) * (pixelCoord.y - c.y)) / denom;
                ...
                if (alpha >= 0 && beta >= 0 && gamma >= 0) {
                    ...
                    if (z < DepthBuffer[idx]) {
                        DepthBuffer[idx] = z;
                        glm::vec3 pos = alpha * c0 + beta * c1 + gamma * c2;
                        glm::vec3 N = glm::normalize(alpha * n0 + beta * n1 + gamma * n2);
                        glm::vec3 L = glm::normalize(light_cam - pos);
                        glm::vec3 Vdir = glm::normalize(-pos);
                        glm::vec3 H = glm::normalize(L + Vdir);
                        float ndotl = glm::dot(N, L);
                        float ndoth = glm::dot(N, H);
                        float diff = std::max(0.0f, ndotl);
                        float spec = std::powf(std::max(0.0f, ndoth), shininess);
                        glm::vec3 color = ka * Ia + kd * lightColor * diff + ks * lightColor * spec;
...
int main() {
    ...
    window = glfwCreateWindow(Width, Height, "Phong Shading (Q3)", NULL, NULL);
    if (!window) {
